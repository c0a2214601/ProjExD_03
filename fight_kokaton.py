...
class Beam:
    ...
    def __init__(self, bird: Bird):
        ...
        self._rct.centerx = bird._rct.centerx + 100  # こうかとんの中心座標＋右
        self._rct.centery = bird._rct.centery  #y座標はそのまま
        self._vx, self._vy = +1, 0
    
    def update(self, screen: pg.surface):
        """
        ビーム速度self._vxに元好き移動させる
        引数　screen:画像Surface
        """
        self._rct.move_ip(self._vx, self._vy)
        screen.blit(self._img, self._rct)
    ...
    while True:
        ...
        if beam is not None:  #ビームが存在しているとき
            ...
            if bomb is not None and beam._rct.colliderect(bomb._rct):
                beam = None
                bomb = None
                bird.change_img(6, screen)  #こうかとんの喜ぶ画像に変える
